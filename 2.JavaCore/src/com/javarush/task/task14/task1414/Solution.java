...
2. Метод MovieFactory.getMovie должен возвращать объект типа Cartoon в случае передачи
ему строки "cartoon" в качестве параметра.
3. Метод MovieFactory.getMovie должен возвращать объект типа Thriller в случае передачи
ему строки "thriller" в качестве параметра.
...
public class Solution {
    public static void main(String[] args) throws Exception {
        ...
        // keys = "cartoon", "thriller", "soapOpera"
        List<String> keys = new ArrayList<String>(Arrays.asList("cartoon", "thriller", "soapOpera"));
        String key = null;
        //Map<String, Movie> resultInputStream = new HashMap<String, Movie>();
        List<Movie> resultInputStream = new ArrayList<Movie>();

        while(true){

            key = bufferedReader.readLine();
            resultInputStream.add(MovieFactory.getMovie(key));
            if(!keys.contains(key))
                break;

        }

        for(Movie movie : resultInputStream) {
